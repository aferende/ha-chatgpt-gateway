...
Separate in-memory limiters use `RATE_LIMIT_MAX` and `RATE_LIMIT_WINDOW_MS`: failed authentication is bucketed by resolved client IP, while successful requests are bucketed by credential ID plus resolved client IP. Invalid callers therefore cannot consume authenticated quota. Set `RATE_LIMIT_MAX=0` only when another trusted limiter protects the endpoint. The separate service-call limiter protects writes with `SERVICE_RATE_LIMIT_MAX` and `SERVICE_RATE_LIMIT_WINDOW_MS`. All maps are bounded and reset on restart. Home Assistant requests have a bounded timeout controlled by `HOME_ASSISTANT_TIMEOUT_MS`.

`TRUSTED_PROXIES` controls Fastify's native `trustProxy` setting. It is empty by default, so forwarded headers from a direct client are ignored and the socket peer remains the rate-limit identity. When an exact proxy peer IP or verified CIDR is configured, Fastify walks only that trusted forwarding chain and uses the first untrusted address as `request.ip`; failed-auth, authenticated, and service limiters then use that client address. A trusted proxy must set or preserve `X-Forwarded-For` correctly. Never configure universal trust, and do not trust a forwarded header merely because it arrived from the Internet.

One structured audit event per protected request contains a server-generated request ID, method, canonical route, status, duration, auth outcome, limiter metadata, and HMAC-pseudonymized client/peer addresses. Raw IPs are off by default. Automatic Fastify request logging is disabled so it cannot bypass that privacy setting. Audit events never contain query strings, request/response bodies, Home Assistant values, tokens, or Authorization headers. See [Forensic observability](forensic-observability.md).
...
The host port mapping is disabled by default. A Raspberry Pi gateway on another host requires an explicit LAN mapping; restrict it to that source with network firewalling where available. `DIAGNOSTICS_ADDON_URL` must use HTTP(S) and point only to the companion on a trusted LAN or private overlay. Public Internet endpoints, Tailscale Funnel, router forwarding, URLs containing credentials/userinfo, query strings, and fragments are not supported. Structural URL validation rejects those URL components but cannot technically determine whether every hostname resolves to a private system; the administrator is responsible for ensuring that the configured target is private and trusted. Plain LAN HTTP does not protect the token from a network observer, so use an isolated trusted network or private encrypted overlay when needed.

Companion lifecycle and request-result audit logs use compact English one-line messages with human-readable UTC timestamps. They contain metadata only: never bearer tokens, Authorization headers, Supervisor response bodies, or returned Core-log content. Gateway audit logs remain structured Pino JSON.
